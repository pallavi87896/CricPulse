...
export default function UserMatchDashboard() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-6 bg-white p-6 sm:p-8 rounded-2xl border border-zinc-200 min-h-[50vh] shadow-xs">
        <div className="h-8 bg-zinc-200 w-1/4 rounded-lg animate-pulse" />
  ...
  const renderScorecard = (match: MatchType, isLive: boolean) => {
    ...
    return (
      <Link key={match._id} href={`/match/${match._id}`} className="block">
        <div className="bg-white border border-zinc-200 hover:border-zinc-355 rounded-2xl p-6 sm:p-8 flex flex-col gap-5 shadow-xs hover:shadow-sm transition-all duration-200 cursor-pointer">
          
          {/* Header line: Venue and status */}
          <div className="flex justify-between items-center text-xs font-bold text-zinc-450 uppercase tracking-wider border-b border-zinc-100 pb-3">
            <span>{match.venue || "Stadium Ground"}</span>
            {isLive ? (
              <span className="text-[var(--color-brand-accent)] bg-[var(--color-brand-secondary)] px-2.5 py-0.5 rounded-md font-extrabold flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-red-500 inline-block animate-pulse" />
                Live • Innings {match.innings}
              </span>
            ) : (
              <span className="text-zinc-500 bg-zinc-100 px-2.5 py-0.5 rounded-md font-extrabold">
                Ended
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 items-center justify-center py-2 sm:py-4 gap-4">
            ...
            <div className="flex flex-col items-center text-center gap-2">
              <TeamLogo logo={match.teamA.logo} name={match.teamA.name} size="md" className="bg-zinc-50" />
              <span className={`text-sm font-extrabold leading-tight ${!isLive && match.winner?._id === match.teamB._id ? "text-zinc-400" : "text-zinc-900"}`}>
                {match.teamA.name}
              </span>
              {isLive && match.battingTeam?._id === match.teamA._id && (
                <span className="text-[9px] font-extrabold text-white bg-[var(--color-brand-accent)] px-2 py-0.5 rounded-full tracking-wider uppercase">
                  Batting
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col items-center justify-center gap-1">
              <div className="text-2xl sm:text-3xl font-black font-mono text-zinc-900 tracking-tight">
                {match.score}/{match.wickets}
              </div>
              <div className="text-xs text-zinc-550 font-mono font-semibold">
            ...
            {/* Team B Details */}
            <div className="flex flex-col items-center text-center gap-2">
              <TeamLogo logo={match.teamB.logo} name={match.teamB.name} size="md" className="bg-zinc-50" />
              <span className={`text-sm font-extrabold leading-tight ${!isLive && match.winner?._id === match.teamA._id ? "text-zinc-400" : "text-zinc-900"}`}>
                {match.teamB.name}
              </span>
          ...
          {isLive ? (
            match.target > 0 ? (
              ...
              match.tossWinner && (
                <div className="bg-zinc-50 border border-zinc-150 p-2 rounded-xl text-center text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
                  {match.tossWinner.name} won the toss and elected to {match.tossDecision} first
                </div>
              )
            )
          ) : (
            match.winner ? (
              ...
              <div className="bg-zinc-50 border border-zinc-150 p-2 rounded-xl text-center text-xs font-bold text-zinc-600">
                Match Finished
              </div>
            )
          )}
  ...
  return (
    <div className="flex flex-col gap-10 max-w-3xl mx-auto py-2">
        ...
        <div className="border-b border-zinc-200 pb-3 text-left">
          <h1 className="text-xl font-extrabold tracking-tight text-zinc-900 flex items-center gap-2">
            ...
            {liveMatches.length > 0 && (
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold bg-red-50 text-red-650 border border-red-100">
                LIVE
              </span>
            )}
          </h1>
      ...
      {/* 2. UPCOMING MATCHES SECTION (Adding Space as requested) */}
              ...
              <Link key={match._id} href={`/match/${match._id}`} className="block">
                <div className="bg-white border border-zinc-200 hover:border-zinc-300 rounded-xl p-5 flex flex-col gap-4 shadow-xs transition-colors">
                  <div className="flex justify-between items-center text-xs text-zinc-400 font-bold">
                    ...
                    <span className="text-[var(--color-brand-accent)] font-extrabold">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-extrabold text-zinc-900">{match.teamA.name}</span>
                    </div>
                    <span className="text-xs text-zinc-450 font-bold px-2">VS</span>
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-extrabold text-zinc-900">{match.teamB.name}</span>
