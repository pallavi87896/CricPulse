  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card title="Recent Matches" subtitle="Overview of newly created, active, or finalized games">
        <SimpleTable
          headers={["Matchup", "Venue", "Status", "Live Score", "Overs",]}
          isEmpty={dashboardData?.recentMatches.length === 0}
        >
          {dashboardData?.recentMatches.map((match) => (
            <tr
              key={match._id}
              onClick={() => router.push(`/admin/match/${match._id}`)}
            ...
            >
              ...
              <td className="px-6 py-4 whitespace-nowrap font-mono font-bold text-zinc-800 text-sm">
                {match.status !== "Upcoming" ? (
                  ...
                  <span className="text-zinc-500 italic text-xs font-semibold">Not Started</span>
                )}
              </td>
              <td className="px-6 py-4 whitespace-nowrap font-mono text-zinc-500 text-xs font-semibold">
                {match.status !== "Upcoming" ? (
                  <span>
